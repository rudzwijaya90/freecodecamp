...
class Player {
  ...
  class Platform {
    constructor (x, y) {
      ...
      this.width = 200;
      this.height = proportionalSize(40) //proportionalSize() function to make sure the height is proportional to the screen size
    }
      draw(){
  ...
  const animate = () => {
    ...
    if (keys.rightKey.pressed && player.position.x < proportionalSize(400)) {
      ...
      if (keys.rightKey.pressed && isCheckpointCollisionDetectionActive) {
        ...
        checkpoints.forEach((checkpoint) => { //step 103
    ...
    checkpoints.forEach((checkpoint, index, checkpoints) => {
      const checkpointDetectionRules = [
        ...
        player.position.x - player.width <= checkpoint.position.x - checkpoint.width + player.width * 0.9, //ensure that the player is close enough to the checkpoint to claim it
